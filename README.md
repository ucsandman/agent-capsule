...
PyPI package (`context_handoff_bundle`) is staged from its local source tree into
...
The default push scope is the `DASHCLAW_*` slice of the detected set. The scan
also finds unrelated live keys (Stripe, GitHub, Resend, Vercel); those are pushed
only if you name them explicitly.
...
in this order: already in `~/capsule` → `gh release download --repo
ucsandman/agent-capsule` → `$CAPSULE_URL` → the dotfiles checkout next to the
script. It skips `apply` when `~/.claude/.capsule-manifest.json` already exists
...
dist/capsule.tgz capsule.mjs --repo ucsandman/agent-capsule`. The repo is
private, so the asset stays private and `gh release download` on the box needs
`--setup_github`. Use `--dry-run` to print the exact `gh` command without
publishing.
